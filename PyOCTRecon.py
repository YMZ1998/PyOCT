...
class OCTImagingProcessing():
    ...
    def __init__(self, root_dir, SampleData=None, Settings=None, sampleID=None, bkgndID=None, Sample_sub_path=None,
                 Bkgnd_sub_path=None, saveOption=False, saveFolder=None, RorC='real', verbose=True, frames=1,
                 alpha2=None, alpha3=None, depths=np.linspace(1024, 2047, 1024), gamma=0.4, wavelegnth=1300,
                 XYConversion=[660, 660],
                 camera_matrix=np.asarray([1.169980E3, 1.310930E-1, -3.823410E-6, -7.178150E-10]), start_frame=1,
                 singlePrecision=True, OptimizingDC=False, ReconstructionMethods='NoCAO'):
        _time1 = time.time()
        ...
        val_tmp = self.Settings['c0'] * (val_p ** 0) + self.Settings['c1'] * (val_p ** 1) + self.Settings['c2'] * (
                    val_p ** 2) + self.Settings['c3'] * (val_p ** 3)
        self.Settings['k'] = 2 * np.pi / val_tmp
        val_ks = self.Settings['NumCameraPix'] / (self.Settings['NumCameraPix'] - 1) * np.abs(
            self.Settings['k'][0] - self.Settings['k'][-1])  # N*dk
        self.Settings['lambda'] = np.mean(
            np.linspace(val_tmp[0], val_tmp[-1], self.Settings['NumCameraPix']))  # central wavelength of OCT beam ]
        ...
        if self.verbose:
            ...
            for key in self.Settings.keys():
                if key == 'depths':
                    print(
                        "{}: {}, {}".format("depths_min_max", np.amin(self.Settings[key]), np.amax(self.Settings[key])))
                else:
        ...
        if SampleData is None:
            self.SampleData = self.readData(self.SamplePath + self.sampleID + "_raw.bin", self.Settings, "data",
                                            self.start_frame, numVoxels)
        else:
        ...
        if self.ReconstructionMethods.lower() == "nocao":
            ...
            """
            self.data = self.Process.dot(
                self.SampleData)  # np.reshape(self.Process.dot(np.reshape(self.SampleData,(Z,X*Y))),(np.size(self.Settings['depths']),X,Y))
            ...
            self.SampleDataTmp = np.fft.ifft(self.SampleData,
                                             axis=0)  # np.reshape(np.fft.ifft(self.SampleData,axis=0),[Z,X,Y]) # to Z-domain
            ...
            self.BkgndDataxl = np.repeat(np.repeat(
                np.median(np.median(np.real(self.SampleDataTmp), axis=1, keepdims=True), axis=2, keepdims=True), X,
                axis=1), Y, axis=2) + 1j * np.repeat(np.repeat(
                np.median(np.median(np.imag(self.SampleDataTmp), axis=1, keepdims=True), axis=2, keepdims=True), X,
                axis=1), Y, axis=2)
            self.BkgndDataxl = np.fft.fft(self.BkgndDataxl, axis=0)
            self.fx = np.repeat(
                np.mean(np.multiply(self.BkgndDataxl, self.SampleData), axis=0, keepdims=True) / np.mean(
                    self.BkgndDataxl ** 2, axis=0, keepdims=True), Z, axis=0)
        ...
        self.DepthProfile = np.squeeze(np.mean(self.OCTData, axis=1,
                                               keepdims=True))  # ,axis=2,keepdims=True)) #np.squeeze(np.mean(np.mean(self.OCTData,axis=1,keepdims=True),axis=2,keepdims=True))
    ...
    def readData(self, filename, Settings, datatype, start_read, readVol):
        ...
        if datatype == "data":
            ...
            raw_string = np.fromfile(fid, dtype='uint16',
                                     count=readVol)  # numpy read file as row-vector, which is different from matlab as column-vector
            Y = int(readVol / (Z * X))
            output = np.double(np.transpose(
                np.reshape(np.swapaxes(np.reshape(raw_string, (Y, X, Z)), 0, 1), (X * Y, Z))))  # now as [Z,X*Y]
    ...
    def OneDimPlot(self, InputLineData_y, InputLineData_x=None, PlotMethod='line', FigTitle=None, XLabel=None,
        ...
        if PlotMethod.lower() == "line":
            ...
            axLine.scatter(InputLineData_x, InputLineData_y, s=10, c='#1f77b4', marker='o', alpha=0.7, linewidths=0.1,
                           edgecolors='face')
        if XLabel:
    ...
    def ShowXZ(self, InputData, frame_toshow=0, figTitle=None, figHandle=None):
        ...
        plt.subplots_adjust(wspace=20, hspace=5, left=0.01,
                            right=0.98)  # fig.subplots_adjust(left=0.05, bottom=0.05, right=0.98, top=0.98)
        if figTitle:
        ...
        axDepth.plot(np.linspace(1, np.size(self.DepthProfile), np.size(self.DepthProfile)), np.abs(self.DepthProfile),
                     linewidth=1, color='C2')
        axSpectrum.plot(np.linspace(1, np.size(self.BkgndData), np.size(self.BkgndData)), np.abs(self.BkgndData),
                        linewidth=1, color='C1', label='bkgnd')
        axSpectrum.plot(np.linspace(1, np.size(self.SampleSpectrum), np.size(self.SampleSpectrum)),
                        np.abs(self.SampleSpectrum), linewidth=1, color='C3', label='signal')
        axIntere.plot(np.linspace(1, np.size(self.InterferencePattern), np.size(self.InterferencePattern)),
                      np.real(self.InterferencePattern), linewidth=1, color='C1')
        ...
        axDepth.set_xlim([np.amin(np.linspace(1, np.size(self.DepthProfile), np.size(self.DepthProfile))),
                          np.amax(np.linspace(1, np.size(self.DepthProfile), np.size(self.DepthProfile)))])
        axSpectrum.set_xlim([np.amin(np.linspace(1, np.size(self.BkgndData), np.size(self.BkgndData))),
                             np.amax(np.linspace(1, np.size(self.BkgndData), np.size(self.BkgndData)))])
        axIntere.set_xlim(
            [np.amin(np.linspace(1, np.size(self.InterferencePattern), np.size(self.InterferencePattern))),
             np.amax(np.linspace(1, np.size(self.InterferencePattern), np.size(self.InterferencePattern)))])
    ...
    def saveData(self):
        ...
        # scipy.io.savemat(savePath+'/'+self.sampleID+"_OCTData.mat",appendmat=False,mdict={"data": self.OCTData})
            ...
            DataFileSave.create_dataset('OCTData_real', shape=np.shape(self.data), data=np.real(self.data),
                                        compression="gzip")
            DataFileSave.create_dataset('OCTData_imag', shape=np.shape(self.data), data=np.imag(self.data),
                                        compression="gzip")
            DataFileSave.close()
...
class Batch_OCTProcessing():
    ...
    def __init__(self, root_dir, ChunkSize=128, Ascans=120, Frames=128, SampleData=None, Settings=None, sampleID=None,
                 bkgndID=None, Sample_sub_path=None, Bkgnd_sub_path=None, saveOption=False, saveFolder=None,
                 RorC='real', verbose=False, alpha2=None, alpha3=None, depths=np.linspace(1024, 2047, 1024), gamma=0.4,
                 wavelegnth=1300, XYConversion=[660, 660],
                 camera_matrix=np.asarray([1.169980E3, 1.310930E-1, -3.823410E-6, -7.178150E-10]), start_frame=1,
                 singlePrecision=True, OptimizingDC=False, ReconstructionMethods='NoCAO', downPrecision=True):
        ...
        for i in range(self.Segments):
            if verbose:
                bar.next()
                print("")
            self.OCTRe = OCTImagingProcessing(root_dir=root_dir, SampleData=SampleData, Settings=None,
                                              sampleID=sampleID, bkgndID=bkgndID, Sample_sub_path=Sample_sub_path,
                                              Bkgnd_sub_path=Bkgnd_sub_path, saveOption=saveOption,
                                              saveFolder=saveFolder, RorC='complex', verbose=False,
                                              frames=self.ChunkSize, alpha2=alpha2, alpha3=alpha3, depths=depths,
                                              gamma=gamma, wavelegnth=wavelegnth, XYConversion=XYConversion,
                                              camera_matrix=camera_matrix, start_frame=i * self.ChunkSize + 1,
                                              singlePrecision=singlePrecision, OptimizingDC=False,
                                              ReconstructionMethods=ReconstructionMethods)
            if i == 0:
                self.Settings = self.OCTRe.Settings
                if verbose:
                    for key in self.Settings.keys():
                        print("{}: {}".format(key, self.Settings[key]))
            self.data[:, :, i * self.ChunkSize + np.arange(0, self.ChunkSize, step=1, dtype=int)] = np.reshape(
                self.OCTRe.data, (np.size(self.Settings['depths']), self.Settings['Ascans'],
                                  int(np.size(self.OCTRe.data) / (
                                              np.size(self.Settings['depths']) * self.Settings['Ascans']))))
        if (self.Segments * self.ChunkSize - self.Frames) != 0:
            ...
            if diff < 0:
                ...
                self.OCTRe = OCTImagingProcessing(root_dir=root_dir, SampleData=SampleData, Settings=None,
                                                  sampleID=sampleID, bkgndID=bkgndID, Sample_sub_path=Sample_sub_path,
                                                  Bkgnd_sub_path=Bkgnd_sub_path, saveOption=saveOption,
                                                  saveFolder=saveFolder, RorC='complex', verbose=False, frames=diff,
                                                  alpha2=alpha2, alpha3=alpha3, depths=depths, gamma=gamma,
                                                  wavelegnth=wavelegnth, XYConversion=XYConversion,
                                                  camera_matrix=camera_matrix,
                                                  start_frame=self.Segments * self.ChunkSize + 1,
                                                  singlePrecision=singlePrecision, OptimizingDC=False,
                                                  ReconstructionMethods=ReconstructionMethods)
                self.data[:, :, (self.Segments) * self.ChunkSize + np.arange(0, diff, step=1, dtype=int)] = np.reshape(
                    self.OCTRe.data, (np.size(self.Settings['depths']), self.Settings['Ascans'],
                                      int(np.size(self.OCTRe.data) / (
                                                  np.size(self.Settings['depths']) * self.Settings['Ascans']))))
        if downPrecision:
