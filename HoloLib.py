...
def HT2D(imgRaw, verbose=False, returnSimple=False, subtract_mean=True, zero_pad=True, **kwargs):
    ...
    **kwargs: arguments 
    ...
    contrast = 2 * np.sum(np.abs(Pimg), dtype=np.float64) / np.sum(np.abs(dc),
                                                                   dtype=np.float64)  # the contrast is defined as the amplitude ratio between interference signal to DC signal, with a factor of 2 accounting to both negative and positive frequencies.
    if verbose:
        ...
        ax2[1, 0].imshow(np.abs(Pimg[:xSizeRaw, :ySizeRaw]) ** 2 / np.abs(dc[:xSizeRaw, :ySizeRaw]) ** 2,
                         cmap="rainbow")
...
def ima2(IMGRaw, verbose=False, subtract_mean=True, zero_pad=True, returnSimple=False, Sref=None, Sref_indx=None,
         **kwargs):
    """
    ...
    if subtract_mean:
        for i in range(szRaw[0]):
            IMGRaw[i, :, :] = IMGRaw[i, :, :] - np.single(np.mean(IMGRaw[i, :, :],
                                                                  dtype=np.float64))  # setting dtype as np.float64 to avoid overflow of summation.
    if zero_pad:
        for i in range(szRaw[0]):
            if i == 0:
    ...
    contrast_mat = np.single(np.squeeze(
        2 * np.sum(np.abs(out_cut), axis=(1, 2), dtype=np.float64) / np.sum(np.abs(DC_cut), axis=(1, 2),
                                                                            dtype=np.float64)))
...
def ShowRecontructedImg(dataRaw, out, mi_raw, mj_raw, trans=False):
    ...
    c1 = plt.Circle(((mi_raw + 0.5) * int(np.shape(dataRaw)[0]), (mj_raw - 0.5) * int(np.shape(dataRaw)[1])), 10,
                    color='g', fill=False, ls=":")
    ax[0, 1].add_patch(c1)
...
def PhaseReconstruction_Batch(inputData, batchSize=50, cr=0.5, trans=False, ref=np.empty((0,)), onlyCPU=False,
                              verbose=False, zero_pad=True, subtract_mean=True, returnContrastMat=False, **kwargs):
    """
    ...
    if (Z < 2) or (onlyCPU):  # now, since padding, GPU always faster than CPU, even for a single frame.
        ...
        for i in range(NumGroup):
            ...
            if np.size(ref) == 0:
                output[i * batchSize + xrange, :, :], _, out_int[i * batchSize + xrange], mi, mj, cmask, contrast_mat[
                    i * batchSize + xrange] = ima2(inputData[i * batchSize + xrange, :, :], cr=cr, verbose=verbose,
                                                   zero_pad=zero_pad, subtract_mean=False, Sref=Sref,
                                                   Sref_indx=Sref_indx, **kwargs)
            else:
                output[i * batchSize + xrange, :, :], _, out_int[i * batchSize + xrange], mi, mj, cmask, contrast_mat[
                    i * batchSize + xrange] = ima2(inputData[i * batchSize + xrange, :, :], verbose=verbose, cr=cr,
                                                   ref=ref, zero_pad=zero_pad, subtract_mean=False, Sref=Sref,
                                                   Sref_indx=Sref_indx, **kwargs)
            # if i == 0:
        ...
        if (Z % batchSize):
            ...
            if np.size(ref) == 0:
                try:
                    output[NumGroup * batchSize + tmp_xrange, :, :], _, out_int[
                        NumGroup * batchSize + tmp_xrange], mi, mj, cmask, contrast_mat[
                        NumGroup * batchSize + tmp_xrange] = ima2(inputData[NumGroup * batchSize + tmp_xrange, :, :],
                                                                  cr=cr, verbose=verbose, zero_pad=zero_pad,
                                                                  subtract_mean=False, Sref=Sref, Sref_indx=Sref_indx, )
                except TypeError:
                    output[NumGroup * batchSize + tmp_xrange, :, :], _, out_int[
                        NumGroup * batchSize + tmp_xrange], mi, mj, cmask, contrast_mat[
                        NumGroup * batchSize + tmp_xrange] = ima2(inputData[NumGroup * batchSize + tmp_xrange, :, :],
                ...
                try:
                    output[NumGroup * batchSize + tmp_xrange, :, :], _, out_int[
                        NumGroup * batchSize + tmp_xrange], mi, mj, cmask, contrast_mat[
                        NumGroup * batchSize + tmp_xrange] = ima2(inputData[NumGroup * batchSize + tmp_xrange, :, :],
                                                                  cr=cr, ref=ref, zero_pad=zero_pad,
                                                                  subtract_mean=False, verbose=verbose, Sref=Sref,
                                                                  Sref_indx=Sref_indx, )
                except TypeError:
                    output[NumGroup * batchSize + tmp_xrange, :, :], _, out_int[
                        NumGroup * batchSize + tmp_xrange], mi, mj, cmask, contrast_mat[
        ...
        for i in range(NumGroup):
            ...
            if np.size(ref) == 0:
                output[i * batchSize + xrange, :, :], _, out_int[i * batchSize + xrange], mi, mj, cmask, contrast_mat[
                    i * batchSize + xrange] = pTH.ima2(inputData[i * batchSize + xrange, :, :], cr=cr, verbose=verbose,
                                                       zero_pad=zero_pad, subtract_mean=False, Sref=Sref,
                                                       Sref_indx=Sref_indx, **kwargs)
            else:
                output[i * batchSize + xrange, :, :], _, out_int[i * batchSize + xrange], mi, mj, cmask, contrast_mat[
                    i * batchSize + xrange] = pTH.ima2(inputData[i * batchSize + xrange, :, :], cr=cr, ref=ref,
                                                       verbose=verbose, zero_pad=zero_pad, subtract_mean=False,
                                                       Sref=Sref, Sref_indx=Sref_indx, **kwargs)
            # if i == 0:
        ...
        if (Z % batchSize):
            ...
            if np.size(ref) == 0:
                try:
                    output[NumGroup * batchSize + tmp_xrange, :, :], _, out_int[
                        NumGroup * batchSize + tmp_xrange], mi, mj, cmask, contrast_mat[
                        NumGroup * batchSize + tmp_xrange] = pTH.ima2(
                        inputData[NumGroup * batchSize + tmp_xrange, :, :], cr=cr, verbose=verbose, subtract_mean=False,
                        zero_pad=zero_pad, Sref=Sref, Sref_indx=Sref_indx, )
                except TypeError:
                    output[NumGroup * batchSize + tmp_xrange, :, :], _, out_int[
                        NumGroup * batchSize + tmp_xrange], mi, mj, cmask, contrast_mat[
                        NumGroup * batchSize + tmp_xrange] = pTH.ima2(
                        inputData[NumGroup * batchSize + tmp_xrange, :, :], cr=cr, verbose=verbose, zero_pad=zero_pad,
                        subtract_mean=False, Sref=Sref, Sref_indx=Sref_indx, **kwargs)
            else:
                try:
                    output[NumGroup * batchSize + tmp_xrange, :, :], _, out_int[
                        NumGroup * batchSize + tmp_xrange], mi, mj, cmask, contrast_mat[
                        NumGroup * batchSize + tmp_xrange] = pTH.ima2(
                        inputData[NumGroup * batchSize + tmp_xrange, :, :], cr=cr, ref=ref, zero_pad=zero_pad,
                        subtract_mean=False, verbose=verbose, Sref=Sref, Sref_indx=Sref_indx, )
                except TypeError:
                    output[NumGroup * batchSize + tmp_xrange, :, :], _, out_int[
                        NumGroup * batchSize + tmp_xrange], mi, mj, cmask, contrast_mat[
                        NumGroup * batchSize + tmp_xrange] = pTH.ima2(
                        inputData[NumGroup * batchSize + tmp_xrange, :, :], cr=cr, ref=ref, verbose=verbose,
                        zero_pad=zero_pad, subtract_mean=False, Sref=Sref, Sref_indx=Sref_indx, **kwargs)
...
def compute_bkgnd(data, fit_offset="mean", fit_profile="tilt", border_m=0, border_perc=0, border_px=0, from_mask=None,
                  ret_mask=False):
...
class QPImage(object):
    ...
    def __init__(self, inData=None, data_key='IMG', ref_data=None, meta_data=None, holo_kw=None, bg_kw=None,
                 proc_phase=False, computeBg=False, slices=0, autoRun=True, verbose=False, **kwargs):
        ...
        self.holo_kw = {"batchSize": 50, "cr": 0.5, "trans": False, "ref": np.empty((0,)), "onlyCPU": False,
                        "verbose": verbose, "zero_pad": False, "subtract_mean": True, "returnContrastMat": True}  #
        if holo_kw is not None:
            for keys in holo_kw:
                self.holo_kw[keys] = holo_kw[keys]

        self.bg_kw = {"fit_offset": "mean", "fit_profile": "tilt", "border_m": 0, "border_perc": 0, "border_px": 80,
            ...
            if autoRun:
                if data.ndim == 3:
                    if slices == 0:
                        ...
                        self.out, self.contrast_mat, self.out_int = self._get_amp_pha(data[0:int(slices), :, :],
                                                                                      **kwargs)
                else:
                    self.out, self.contrast_mat, self.out_int = self._get_amp_pha(data, **kwargs)
    ...
    def _get_amp_pha(self, data, **kwargs):
        ...
        if data.ndim == 2:
            out = HT2D(data, verbose=self.holo_kw["verbose"], returnSimple=True,
                       subtract_mean=self.holo_kw["subtract_mean"], zero_pad=self.holo_kw["zero_pad"], **kwargs)
            return out, np.zeros((np.shape(out)[0],)), np.zeros((np.shape(out)[0],))
        else:
            out, contrast_mat, out_int = PhaseReconstruction_Batch(data, batchSize=self.holo_kw["batchSize"],
                                                                   cr=self.holo_kw["cr"], trans=self.holo_kw["trans"],
                                                                   ref=self.holo_kw["ref"],
                                                                   onlyCPU=self.holo_kw["onlyCPU"],
                                                                   verbose=self.holo_kw["verbose"],
                                                                   zero_pad=self.holo_kw["zero_pad"],
                                                                   subtract_mean=self.holo_kw["subtract_mean"],
                                                                   returnContrastMat=True, **kwargs)
    ...
    def compute_bg(self):
        if self.bg_kw["ret_mask"]:
            self.bg, self.mask = compute_bkgnd(self.pha, fit_offset=self.bg_kw["fit_offset"],
                                               fit_profile=self.bg_kw["fit_profile"], border_m=self.bg_kw["border_m"],
                                               border_perc=self.bg_kw["border_perc"], border_px=self.bg_kw["border_px"],
                                               from_mask=self.bg_kw["from_mask"], ret_mask=self.bg_kw["ret_mask"])
            if self.subtract_ref:
                ref_out_bkgnd, _ = compute_bkgnd(self.ref_out, fit_offset=self.bg_kw["fit_offset"],
                                                 fit_profile=self.bg_kw["fit_profile"], border_m=self.bg_kw["border_m"],
                                                 border_perc=self.bg_kw["border_perc"],
                                                 border_px=self.bg_kw["border_px"], from_mask=self.bg_kw["from_mask"],
                                                 ret_mask=self.bg_kw["ret_mask"])
        else:
            self.bg = compute_bkgnd(self.pha, fit_offset=self.bg_kw["fit_offset"],
                                    fit_profile=self.bg_kw["fit_profile"], border_m=self.bg_kw["border_m"],
                                    border_perc=self.bg_kw["border_perc"], border_px=self.bg_kw["border_px"],
                                    from_mask=self.bg_kw["from_mask"], ret_mask=self.bg_kw["ret_mask"])
            if self.subtract_ref:
                ref_out_bkgnd = compute_bkgnd(self.ref_out, fit_offset=self.bg_kw["fit_offset"],
                                              fit_profile=self.bg_kw["fit_profile"], border_m=self.bg_kw["border_m"],
                                              border_perc=self.bg_kw["border_perc"], border_px=self.bg_kw["border_px"],
                                              from_mask=self.bg_kw["from_mask"], ret_mask=self.bg_kw["ret_mask"])
