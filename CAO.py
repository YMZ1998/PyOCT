...
def QuadraticFit2d(inData, Settings, verbose=False):
    ...
    X = X.flatten()
    Y = Y.flatten()
    A = np.array([X * 0 + 1, X, Y, X ** 2, X ** 2 * Y, X ** 2 * Y ** 2, Y ** 2, X * Y ** 2,
                  X * Y]).T  # np.array([X*0+1, X, Y, X**2,  Y**2, X*Y]).T
    B = surfData.flatten()
    coeff, r, rank, _ = np.linalg.lstsq(A, B,
                                        rcond=-1)  # rcond = -1, using machine precision as to define zeros. rcond = None means that values smaller than machine precision * Max of A will be considered as zeros.
    if verbose:
        print("QuadraticFit2d results:")
        print("coeff0: {}".format(coeff[0]))
        print("r : {}".format(r))
        print("rank: {}".format(rank))
    zxy = coeff[1] * X + coeff[2] * Y + coeff[3] * X ** 2 + coeff[4] * X ** 2 * Y + coeff[5] * X ** 2 * Y ** 2 + coeff[
        6] * Y ** 2 + coeff[7] * X * Y ** 2 + coeff[
              8] * X * Y  # coeff[1]*X+coeff[2]*Y+coeff[3]*(X**2) + coeff[4]*(Y**2)+coeff[5]*X*Y
...
def SearchingCoverGlass(inData, Settings, start_index=5, end_index=150, verbose=False):
    ...
    if verbose:
        ...
        axSG.plot(tempData)
        axSG.scatter(peaks[0], tempData[peaks[0]], s=80, facecolors='none', edgecolors='tab:red', linewidths=2,
                     linestyles='dotted')
        axSG.scatter(peaks[1], tempData[peaks[1]], s=80, facecolors='none', edgecolors='tab:red', linewidths=2,
                     linestyles='dotted')
        axSG.set_yscale('log')
...
def PhaseRegistration(inData, Settings, dzc=5, start_index=5, end_index=150, verbose=False):
    ...
    surface_phi = np.average(inData[pos_coverglass - dzc:pos_coverglass + dzc, :, :], axis=0,
                             weights=np.abs(inData[pos_coverglass - dzc:pos_coverglass + dzc, :, :]))
...
## S3: Defocus 
def SearchingFocalPlane(inData, Settings, start_bias=50, extend_num_pixels=240, start_index=5, end_index=150,
                        showFitResults=False, smethod="per"):
    """
    ...
    if smethod.lower() == '2dfit':
        zf, _ = QuadraticFit2d(
            (inData[(pos_cover + start_bias):(pos_cover + start_bias + extend_num_pixels), :, :]) ** 0.4, Settings,
            verbose=showFitResults)
        zf = int(zf + pos_cover + start_bias)  # now this will serve as the reference position of cover glass
    elif smethod.lower() == 'max':
        zf = int(np.argmax(np.squeeze(np.amax(
            np.amax(np.abs(inData[(pos_cover + start_bias):(pos_cover + start_bias + extend_num_pixels), :, :]), axis=1,
                    keepdims=True), axis=2, keepdims=2))))
        ...
        sizeTmp = X * Y
        zprofile = np.squeeze(np.median(tmp[(pos_cover + start_bias):(pos_cover + start_bias + extend_num_pixels), int(
            sizeTmp * 0.996):int(sizeTmp * 0.999)], axis=1))
...
def ViolentDefocus(inData, Settings, showFitResults=False, proctype='oce', verbose=False, start_bias=50,
                   extend_num_pixels=240):
    if verbose:
        print("Defocusing...")
    # searching focal plane, regardless zf is known or not
    if 'zf' not in Settings.keys():
        zfpos, Settings = SearchingFocalPlane(inData, Settings, showFitResults=showFitResults, start_bias=start_bias,
                                              extend_num_pixels=extend_num_pixels)
...
def CoherenceGateRemove(inData, Settings, proctype='OCE', verbose=False, start_index=5, end_index=150):
    ...
    # aperture = np.ones(np.shape(qr))
    # defocus kernel
    qz = np.sqrt(np.multiply(aperture, (2 * Settings[
        'refractive_index'] * kc) ** 2 - qr ** 2))  # np.sqrt((2*Settings['refractive_index']*kc)**2-qr**2)
...
def FocalPlaneRegistration(inData, Settings, proctype='OCT', showFitResults=False, start_bias=50, extend_num_pixels=240,
                           verbose=False):
    if 'zf' not in Settings.keys():
        _, Settings = SearchingFocalPlane(inData, Settings, showFitResults=showFitResults, start_bias=start_bias,
                                          extend_num_pixels=extend_num_pixels)
    zfpos = int(Settings['zf'] / Settings['zPixSize'])
...
def Gauss2(x, amp1, cen1, wid1, amp2, cen2, wid2):
    ...
    return amp1 * np.exp(- (x - cen1) ** 2.0 / (2.0 * wid1 ** 2.0)) + amp2 * np.exp(
        - (x - cen2) ** 2.0 / (2.0 * wid2 ** 2.0))
...
def FullCAO(inData, Settings, verbose=False, proctype='oct', start_index=5, end_index=200, start_bias=100,
            extend_num_pixels=150, singlePrecision=False, excludeGlass=True):
    """
    ...
    inData, Settings.
    """
    inData, Settings = CoherenceGateRemove(inData, Settings, verbose=verbose, proctype=proctype,
    ...
    inData, Settings = ViolentDefocus(inData, Settings, showFitResults=verbose, proctype=proctype,
                                      start_bias=start_bias, extend_num_pixels=extend_num_pixels)
    if singlePrecision:
