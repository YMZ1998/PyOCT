...
if __name__ == "__main__":
    ...
    if Test:
        ...
        if im.ndim == 3:
            ...
            img = np.asarray(im, dtype=np.float64) / np.iinfo(im.dtype).max
    ...
    backend_name = "CUDA" if CUDA_AVAILABLE else "NumPy CPU fallback"
    print("{} FFT output size {}".format(backend_name, np.shape(fft2)))
    print("FFT maximum absolute error: {:.3e}".format(np.max(np.abs(fft2 - fft1))))
    ...
    ax[1].set_title("FFT difference ({})".format(backend_name))

    ifft1 = np.fft.ifft2(np.fft.ifftshift(fft1))
    ifft2 = ifft2_gpu(fft2, fftshift=True)
    print("Round-trip maximum absolute error: {:.3e}".format(np.max(np.abs(ifft2 - img))))
    ...
    ax1[0, 1].set_title("iFFT ({})".format(backend_name))
