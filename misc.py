...
def ListAllDataFile(data_path, endsWith, startsWith=None, searchPattern=r"_\d+_", searchPattern2=r"\d+",
                    returnNum=False):
    """
...
def SaveData(save_path, FileName, inData, datatype='data', varName='OCTData'):
    ...
    if datatype.lower() == 'data':
        if np.iscomplexobj(inData):
            ...
            DataFileSave.create_dataset(varName + '_real', shape=np.shape(inData), data=np.real(inData),
                                        compression="gzip")
            DataFileSave.create_dataset(varName + '_imag', shape=np.shape(inData), data=np.imag(inData),
                                        compression="gzip")
            DataFileSave.close()
        else:
            DataFileSave = h5py.File(save_path + '/' + FileName + '.hdf5', 'w')
...
def patternMatch(template, rootImage, cropIndex=None, showFit=False):
    ...
    if cropIndex == None:
        ...
        centerofCropInTemplate = (int(np.ceil((cropIndex[1] + cropIndex[0]) / 2)),
                                  int(np.ceil((cropIndex[2] + cropIndex[3]) / 2)))
    ...
    if showFit:
        ...
        rect = patches.Rectangle((cPos[1] - np.shape(CropImage)[1] / 2, cPos[0] - np.shape(CropImage)[0] / 2),
                                 np.shape(CropImage)[1], np.shape(CropImage)[0], fill=False, linestyle='--',
...
def WriteIntoGif(path, fps, endsWith='.png', saveFileName=None, inFormat="mp4", count=-1):
    ...
    if inFormat == "gif":
        ...
        writer = imageio.get_writer(path + "/" + "animation.mp4", format="FFMPEG",
                                    **kwargs)  # pixelformat='vaapi_vld',
        for ii in range(len(pngFiles)):
            bar2.next()
            tmpImg = imageio.imread(path + '/' + pngFiles[ii])
            if ii == 0:
...
def patternMatch_fft_scan(testVol, refPlane, cAxis, numAve=2, showFit=False):
    ...
    refPlane = (refPlane - np.mean(refPlane)) / np.std(
        refPlane)  # normalize to reduce the effects of variant brightness on motion correction
...
def create_fig(figsize=[3, 2], nrows=1, ncols=1, left=0.12, bottom=0.12, right=0.95, top=0.95, sharex=False,
               sharey=False, wspace=0.2, hspace=0.2):
...
def show3DStack(image_3d, axis=0, cmap="gray", extent=(0, 1, 0, 1), trans=False, addPatch=False, NewPatch=None,
                figTitle="Img", excludeNaN=True, **kwargs):
    if axis == 0:
    ...
    if "clim" in kwargs.keys():
        ...
        if trans:
            fig = ax.imshow(np.transpose(image(current_idx)), cmap=cmap,
                            clim=(np.nanmin(image(current_idx)), np.nanmax(image(current_idx))), extent=extent)
        else:
            fig = ax.imshow(image(current_idx), cmap=cmap,
                            clim=(np.nanmin(image(current_idx)), np.nanmax(image(current_idx))), extent=extent)
...
def quandraticFit2d(input, verbose=False):
    ...
    X = X.flatten()
    Y = Y.flatten()
    A = np.array([X * 0 + 1, X, Y, X ** 2, Y ** 2,
                  X * Y]).T  # np.array([X*0+1, X, Y, X**2, X**2*Y, X**2*Y**2, Y**2, X*Y**2, X*Y]).T ##
    B = img.flatten()
    coeff, r, rank, _ = np.linalg.lstsq(A, B, rcond=-1)
    zxy = coeff[1] * X + coeff[2] * Y + coeff[3] * (X ** 2) + coeff[4] * (Y ** 2) + coeff[
        5] * X * Y  # coeff[1]*X+coeff[2]*Y+coeff[3]*X**2 + coeff[4]*X**2*Y+coeff[5]*X**2*Y**2+coeff[6]*Y**2+coeff[7]*X*Y**2+coeff[8]*X*Y #
